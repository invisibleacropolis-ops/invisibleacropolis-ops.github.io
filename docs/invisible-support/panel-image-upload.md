...
## Purpose
The Image upload panel accepts one or more image files, validates each file, uploads the payload to GitHub-backed storage, persists metadata to the image store, and updates related UI surfaces (gallery + viewer). The controller is initialized by `init()` in `public/InvisibleSupport/src/features/images/upload.js` and binds to form-level data selectors.

## DOM selectors used by the module
`public/InvisibleSupport/src/features/images/upload.js` queries and controls the following selectors:
...
- **Primary module:** `public/InvisibleSupport/src/features/images/upload.js`
